...
<body>
  ...
  <form class="d-block p-2" method="post" action="{{ route('id', ['id'=>Auth::user()->id]) }}">
  　  <input type="hidden" name="_token" value="{{csrf_token()}}">
　　　{{-- 隠しフィールド --}}
　　　<input type="hidden" name="_method" value="PATCH">
　　 <p>{{ Auth::user()->name }}の情報を登録</p>
      <p>最大周波数</p>
      <input type="text" value="{{Auth::user()->max_f}}" name="max_f">
      <p>最大周波数平均</p>
      <input type="text" value="{{Auth::user()->max_average_f}}" name="max_average_f">
